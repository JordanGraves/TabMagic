...
void AudacityProject::CreateMenusAndCommands()
{
   ...
   c->BeginMenu(_("&My Menu"));
   /// Generate
   c->AddItem(wxT("Generate"), _("&Generate..."), FN(OnGenerate));
...
void AudacityProject::OnGenerate()
{
  std::cout << "Generate! "<< std::endl;
 
  //Open Files to write notevals
  std::ofstream notevals;
  notevals.open("notevals.txt");
  
  int tempo;
  gPrefs->Write(wxT("/myTempo"), wxString(wxT("120")));
  /** Just to make sure it's retrieveable **/
  wxString myTempo;
  myTempo = gPrefs->Read(wxT("/myTempo"), wxT(""));
  /** And we convert to an int here */
  int num;
  num = wxAtoi(myTempo);
  double blockSize = 60.0/num;

  TrackListIterator iter(mTracks);
  Track *t = iter.First();
  if(t) {	// To make sure there is actually a track
    


    std::string trackName = std::string(t->GetName().mb_str());
    double length = t->GetEndTime() - t->GetStartTime();
    std::cout << "- Track Name "<< trackName << std::endl;
    std::cout << "- Track Length: " << length << " seconds" << std::endl;
    ...
    if (t->GetKind() == Track::Wave) {
      ...
      wxString wxSampleRate = gPrefs->Read(wxT("/SamplingRate/DefaultProjectSampleRate"), wxT(""));
      int samplerate = wxAtoi(wxSampleRate);
      std::cout << "-samplerate: " << samplerate << std::endl;
      
      double dBRange = wxAtoi( gPrefs->Read(wxT("/GUI/EnvdBRange"), wxT("")) );
      
      
      
      Analyzer *analyzer = new Analyzer(num, samplerate, dBRange, track);
      std::cout << "Calling Process" << std::endl;
      analyzer->process();
       std::cout << "End Process" << std::endl;
      /** ** ** ** ** ** ** **  ** ** ** ** 
      
		Determine # of blocks to use
	
		(We are NOT catching the last block)
	  
     ** ** ** ** ** ** ** ** ** ** ** **/ 
//       int numBlocks = length/blockSize;
//       
//       sampleCount start, end;
//       AudacityProject *ip = GetActiveProject();
//       start = track->TimeToLongSamples(ip->mViewInfo.sel0);
//       end = length * samplerate;
//       /** Loop: Loops through track a blockSize at a timmae1
//       *		mstart is at 0
//       * 	mend = length of track
//       **/
//       float begin = 0.0;
//       int count = 0;
//       while(begin + blockSize < length) {		// Add blocksize because we are not doing the last block
// 	  start = begin * samplerate;
// 	  end = start + blockSize*samplerate;
// 	  
// 
// 	  count++;
// 	  begin += blockSize;
// 	  std::cout << "Block 1 - begin: " << begin << " start: " << start << " end: " << end << std::endl; 
//       
//       
//       
//       // Code that needs to be changes for loop
//      /* sampleCount start, end;
//       AudacityProject *p = GetActiveProject();
//       start = track->TimeToLongSamples(p->mViewInfo.sel0);
//       end = samplerate*length;
//       */
//       
//       
//       std::cout << "-start: " << start << "  end: " << end << std::endl;
//       int mDataLen = (sampleCount)(end - start);
//       std::cout << "-DataLen: " << mDataLen << std::endl;
//       float *buffer2 = new float[mDataLen];
//       track->Get((samplePtr)buffer2, floatSample, start, mDataLen);
//       float *mBuffer = new float[mDataLen];;
...
//        std::ofstream f;
//        f.open("getAudio.txt");
//        f << "start: " << start  << "  end: " << end <<"  mDataLen: " << mDataLen << std::endl; 
//        //  start
//        //  end
//        //  mDataLen
//        
...
//        long mWindowSize = 4096;
//        
//        
//        int half = mWindowSize / 2;
//        std::cout << std::endl << "Analyzing... " << " Algorithm: " << alg << " Function: " << func << std::endl;
//               
...
//        float *mData = new float[mDataLen];
//        
//        
//        //Plot()
...
//        while (jstart + mWindowSize <= mDataLen) {
//          for (i = 0; i < mWindowSize; i++)
//             in[i] = win[i] * mData[jstart + i];
...
//              mYMin = mProcessed[i];
// 	  myfile << i << " : " << mProcessed[i] << std::endl;
//         }
//         if(mYMin < -dBRange)
// 	   mYMin = -dBRange;
// 	if(mYMax <= -dBRange)
...
//        
//        //DrawPlot();
// 	/**     wxWidgets stuff, for the rectangle         **/
// 	/**     End wxWidgets stuff...I hope..   **/
// 	
// 	
...
// 	    xPos *= xStep;
// 	}
// 	std::cout << "FOUND LOUDEST FREQ: " << std::endl; 
// 	std::cout << "loudest_freq: " << loudest_freq << "   dB: " << highest_db << std::endl;
//         
// 	wxString xpitch;
// 	xpitch = PitchName_Absolute(FreqToMIDInote(loudest_freq));
// 	
// 	std::string note = std::string(xpitch.mb_str());
// 	std::cout << "NOTE: " << note << std::endl  << std::endl << std::endl;
// 	notevals << "Start Time: " << start/(float)samplerate << "         EndTime: " << float(start+blockSize)/(float)samplerate <<
// 		    "       Note: " << note <<  "       dB: " << highest_db << std::endl;
// 	/// Here is where we will close the for each block loop
// 	
//         delete[] mBuffer;
//       }
    }
