...
BEGIN_EVENT_TABLE(TempoDialog, wxDialog)
   EVT_BUTTON(wxID_OK, TempoDialog::OnOK)
   EVT_BUTTON(wxID_CANCEL, TempoDialog::OnCancel)
   EVT_TREE_KEY_DOWN(wxID_ANY, TempoDialog::OnTreeKeyDown) // Handles key events when tree has focus
END_EVENT_TABLE()
...
TempoDialog::TempoDialog(wxWindow * parent)
...
void TempoDialog::OnCancel(wxCommandEvent & WXUNUSED(event))
{
...
void TempoDialog::OnTreeKeyDown(wxTreeEvent & event)
{
...
void TempoDialog::OnOK(wxCommandEvent & WXUNUSED(event))
{
...
void TempoDialog::SelectPageByName(wxString pageName)
{
...
void TempoDialog::ShowTempDirPage()
{
