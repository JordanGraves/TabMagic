...
void FreqWindow::GetAudio()
{
   ...
   while (t) {
      if (t->GetSelected() && t->GetKind() == Track::Wave) {
	    ...
	    file.open("getAudio1.txt");
	    file << "start: " << start << "  end: " << end <<"  mDataLen: " << mDataLen << std::endl; 
...
void FreqWindow::DrawPlot()
{
   ...
   int width = r.width - 2;
   std::cout << " The width we are looking for " << width << std::endl;
   ...
   std::cout << "Their Function" << std::endl;
   std::cout << "check vals before ProcessedValue()" << std::endl;
   std::cout << "width " << width << std::endl;
   std::cout << "xPos " << xPos << std::endl;
   std::cout << "xStep " << xStep << std::endl;
   std::cout << "mWindowSize " << mWindowSize << std::endl;
   std::cout << "mRate " << mRate << std::endl;
   std::cout << "mProcessedSize" << mProcessedSize << std::endl;
   ...
   for (i = 0; i < width; i++) {
      ...
      /* JG */
      //std::cout << "xPos:" << xPos << "y: " << y << std::endl;
      if (y > highest_db) {
	loudest_freq = xPos;
	highest_db = y;
      }
   ...
   /****************************************************************************************/
   /* JG */
   // This is where we can do our loop //
   std::cout << "iterations: " << i << std::endl; 
   std::cout << "loudest_freq: " << loudest_freq << std::endl; 
   std::cout << "Running!!!" << std::endl; 
   Block* b = new Block(loudest_freq, highest_db);
   {
   ...
   /**---------------------------------------------------------------------------*/
   /// Create a block with the info from the spectrum
...
void FreqWindow::PlotPaint(wxPaintEvent & evt)
{
   ...
   if ( r.Contains(mMouseX, mMouseY) & (mMouseX!=0) & (mMouseX!=r.width-1) ) {
	 ...
	 /*JG*/
	 std::cout << "Cursor(freq): " << int (xPos + 0.5) << "  value(dB):" << value << std::endl;
	 ...
	 /*JG
	  */
	 std::cout << "Cursor: " << xPos << "  value:" << value <<  "   bestpeak: " << bestpeak << std::endl;
